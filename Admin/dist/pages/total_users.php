...
// Fetch Total Records
$recordsPerPage = isset($_GET['entries']) ? (int)$_GET['entries'] : 10; // Number of records per page
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1; // Current page
$page = max(1, $page); // Ensure page is at least 1
$offset = ($page - 1) * $recordsPerPage;

// Count total records
$totalRecordsQuery = "SELECT COUNT(*) AS total FROM users WHERE role = 'user'";
...
$sql = "SELECT id, name, username, email, image, is_active FROM users WHERE role = 'user' LIMIT $recordsPerPage OFFSET $offset";
...
if (isset($_GET['fetch_all'])) {
    $query = "SELECT id, name, username, email, image, is_active FROM users WHERE role = 'user'";
...
<html lang="en">
  ...
  <head>
    ...
    .entries-section {
        ...
        .entries-section select:hover {
            border-color: #c43b68;
        }
        .entries-section select:focus {
            outline: none;
            border-color: #c43b68;
            box-shadow: 0 0 0 3px rgba(196, 59, 104, 0.1);
        }
  ...
  <body class="layout-fixed sidebar-expand-lg bg-body-tertiary">
    ...
    // Get active users
    $active_users_query = "SELECT COUNT(*) as active FROM users WHERE role = 'user' AND is_active = '1'";
    $active_users_result = mysqli_query($conn, $active_users_query);
    $active_users = mysqli_fetch_assoc($active_users_result)['active'];

    // Get inactive users
    $inactive_users_query = "SELECT COUNT(*) as inactive FROM users WHERE role = 'user' AND is_active = '0'";
    $inactive_users_result = mysqli_query($conn, $inactive_users_query);
    $inactive_users = mysqli_fetch_assoc($inactive_users_result)['inactive'];

    // Get new users (last 7 days)
    $new_users_query = "SELECT COUNT(*) as new_users FROM users WHERE role = 'user' AND join_date >= DATE_SUB(NOW(), INTERVAL 7 DAY)";
    ...
    <div style="display: flex; align-items: center; gap: 5px;">
        ...
        <div id="bulkActionControls" style="display: none; margin-right: 15px;">
            ...
            <button id="bulkActivateBtn" class="bulk-action-btn" onclick="bulkToggleStatus('active')" style="
            ...
            <button id="bulkDeactivateBtn" class="bulk-action-btn" onclick="bulkToggleStatus('inactive')" style="
...
?>
        ...
        <!-- Add showing entries info -->
        <div class="showing-entries">
            Showing <strong><?php echo min(($page - 1) * $recordsPerPage + 1, $totalRecords); ?></strong> to 
            <strong><?php echo min($page * $recordsPerPage, $totalRecords); ?></strong> of <strong><?php echo $totalRecords; ?></strong> entries
        </div>
    ...
    <?php if ($totalPages > 1): ?>
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <a href="total_users.php?page=<?= $i ?>&entries=<?= $recordsPerPage ?>" 
...
function updateBulkActionControls() {
    ...
    if (checkboxes.length > 0) {
        ...
        bulkToggleBtn.innerHTML = `<i class="fa-solid fa-toggle-${allActive ? 'off' : 'on'}"></i> ${allActive ? 'Deactivate' : 'Activate'} Selected`;
    } else {
...
async function bulkToggleStatus(status) {
    ...
    if (confirm(`Are you sure you want to ${statusText} ${selectedUsers.length} selected users?`)) {
        try {
            ...
            const result = await response.json();
            if (result.success) {
                alert(`Selected users ${status}d successfully`);
                location.reload();
            } else {
...
function changeEntries() {
    ...
    currentUrl.searchParams.set('page', '1'); // Reset to first page when changing entries
    ...
    <script
      ...
      integrity="sha256-+vh8GkaU7C9/wbSLIcwqSB1/Rf9WtqRHgG5S0="
      crossorigin="anonymous"
    ></script>
    <!-- ChartJS -->
    <script>
    ...
    <script
      ...
      integrity="sha256-tzhRjVqNSRyWg2wbppGnT833E/Ys0DHWGwT04GiqQg="
      crossorigin="anonymous"
    ></script>
    <script
